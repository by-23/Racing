...
namespace Ilumisoft.SkillDrive.Effects
{
    ...
    {
        [SerializeField] [Tooltip("Max angle used to rotate the wheels")]
        float maxAngle = 30.0f;

        [SerializeField] [Tooltip("Optional offset applied to the rotation")]
        ...
        void Update()
        {
            if (Vehicle != null)
            {
                ...
                Quaternion targetRotation = Vehicle.transform.rotation * Quaternion.Euler(0f,
                    rotationOffset + maxAngle * UnityEngine.Input.GetAxis("Vertical"), 0);
